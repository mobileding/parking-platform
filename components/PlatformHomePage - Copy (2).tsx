...
const FEATURED_DOMAINS = [
  ...
  { name: "testdomain.local", status: "For Sale" }, // Good for your local testing
...
export default function PlatformHomePage() {
  ...
  return (
    <main className="min-h-screen bg-white text-gray-900 font-sans">
      ...
      {/* --- 💡 NEW: Live Domain Showcase (Scrolling List) --- */}
        ...
        <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide justify-start md:justify-center">
                {FEATURED_DOMAINS.map((item) => (
                    <a 
                        key={item.name}
                        // NOTE: Adding :3000 for local dev convenience. Remove in prod if needed.
                        href={`http://${item.name}:3000`} 
                        target="_blank" 
                        rel="noopener noreferrer"
                        className="flex-shrink-0 group relative bg-white px-6 py-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all duration-200 min-w-[220px]"
                    >
                        <div className="flex flex-col items-start">
                            ...
                            <div className="flex items-center gap-2 mt-2">
                                <span className={`w-2 h-2 rounded-full ${item.status === 'For Sale' ? 'bg-green-500' : 'bg-yellow-500'}`}></span>
        ...
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
            ...
            <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 hover:border-gray-600 transition">
              ...
              <p className="text-gray-400">Upload your domains manually or via CSV and point nameservers.</p>
            </div>
            <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 hover:border-gray-600 transition">
              <span className="text-indigo-400 font-bold text-lg">Step 3</span>
              <h4 className="text-xl font-bold mt-2 mb-3">Go Live</h4>
              <p className="text-gray-400">Your domains instantly show beautiful, inspirational landing pages.</p>
            </div>
          </div>
